"""Calculate the PhenoLag: time between clinical description and genetic verification.

Cohort definitions
------------------
- **Universe**: Orphanet disorders of type 'Disease' with at least one gene link
  in `disorder_genes` (any source).
- **Core cohort** (`in_core_cohort = 1`): disorders with a G2P or ClinGen link.
  These are the diseases with curated, validated gene–disease associations and
  form the main sample for lag analysis. HPO-only links are weaker (they
  include candidate genes propagated from phenotype overlap) and are kept
  for enrichment only.

Dating priority
---------------
Genetic verification year:
  - Earliest PMID from G2P publications for the selected gene–disease pair.

Clinical description year, in priority order (recorded in `clin_source`):
  1. OMIM_elink           — earliest PMID cited by the OMIM entry (best)
  2. HPO_non_g2p          — earliest HPO-annotation PMID not in the G2P set
  3. HPO_fallback         — earliest HPO PMID when non-g2p set is empty
  4. MIM_breakpoint       — rough year from MIM-number breakpoints (weakest)

Suspect flags
-------------
A record is flagged `suspect_dating = 1` (with a reason string) when the
reported lag is unsafe to put on a poster without manual review:
  - `neg_lag`:         clin_year > gen_year (clinical description appears
                        AFTER genetic verification — almost always a HPO
                        citation artefact, e.g. HPO annotating a review
                        paper instead of the first description).
  - `pre_medline`:     clin_year < 1946 (MEDLINE indexing starts here; earlier
                        years rely on fragile historical OMIM references).
  - `mim_breakpoint`:  clin_source is MIM_breakpoint (no real publication).
  - `codiscovery`:     clin_source is HPO_fallback — happens when every HPO
                        annotation for this disease cites the same paper(s)
                        as G2P, so "clinical" and "genetic" year collapse to
                        the same publication. Empirically gives lag = 0 for
                        essentially every record; not a real clinical-first
                        description, kept out of the lag distribution.

The main poster figure uses only: in_core_cohort = 1 AND suspect_dating = 0
AND lag_years is not NULL.
"""
