"""Poster-ready visualizations for PhenoLag.

Principles:
  - One figure = one message. No dashboards.
  - All numerical figures use the MAIN SAMPLE:
        in_core_cohort = 1 AND suspect_dating = 0 AND lag_years IS NOT NULL.
  - Cohort cascade and historical-review figures are rendered even when the
    main sample is empty — they explain where diseases dropped out.

Figures produced (in order, viz step):
  fig_cohort_cascade      — N waterfall from Orphanet to main sample
  fig1_lag_distribution   — histogram + KDE of lag on the main sample
  fig2_lag_by_era         — lag vs decade of first clinical description
  fig3_timeline_scatter   — clinical year vs genetic year, color = lag
  fig4_cumulative         — cumulative discoveries: clinical vs genetic
  fig5_lag_by_class       — lag by Orphanet disease category
  fig6_inheritance_lag    — lag by inheritance pattern
  fig7_constraint_vs_lag  — LOEUF / pLI vs lag
  fig8_hpo_complexity     — HPO term count vs lag
  fig9_confidence_lag     — G2P evidence confidence vs lag (core subset)
  fig10_clin_source       — breakdown of clinical-year source
  fig11_top_lag           — top 20 diseases with longest odyssey
  fig12_decade_rate       — clinical vs genetic discovery rate per decade
  fig_historical_review   — dating-flag counts + historical_review.csv export
"""
