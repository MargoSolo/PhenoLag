"""Fetch clinical description dates from OMIM via NCBI elink.

For each OMIM MIM number, uses elink (omim -> pubmed) to get all
cited PMIDs, then finds the earliest publication year. This gives
a much more accurate "first clinical description" date than the
MIM number approximation.

Rate limiting: with API key NCBI allows 10 req/s.
elink + efetch (inside get_years) = 2 requests per OMIM entry.
We batch: first collect all PMIDs via elink, then fetch years in bulk.
"""
