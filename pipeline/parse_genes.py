"""Parse gene-related files: HGNC, phenotype_to_genes, G2P, ClinGen, gnomAD constraints.

Disorder–gene links go into `disorder_genes` with a `source` tag:
  - 'G2P'      — validated gene-disease curation (EBI Gene2Phenotype)
  - 'ClinGen'  — ClinGen gene curation / dosage sensitivity with MONDO ID
  - 'HPO'      — phenotype_to_genes.txt (weakest: HPO aggregation, includes candidates)

A disease is in the CORE COHORT iff it has at least one G2P or ClinGen link.
HPO-only links are kept for enrichment but excluded from core analyses.
"""
import csv
import pandas as pd
